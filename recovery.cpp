...
browse_directory(const char* path, Device* device) {
    ...
    const char* MENU_HEADERS[] = { "请选择一个刷机包：",
                                   path,
                                   "",
                                   NULL };
    DIR* d;
...
wipe_media(int confirm, Device* device) {
    if (confirm) {
        ...
        if (title_headers == NULL) {
            const char* headers[] = { "是否擦除内置存储的所有数据?",
                                      "  执行擦除操作后无法恢复原本数据.",
                                      "",
                                      NULL };
            title_headers = prepend_title((const char**)headers);
...
static int enter_sideload_mode(int* wipe_cache, Device* device) {
    ...
    static const char* headers[] = {  "Adb 线刷模式",
                                "",
                                NULL
    };

    static const char* list[] = { "退出 Adb 线刷模式", NULL };
...
show_apply_update_menu(Device* device) {
    ...
    if (chosen == item_sideload) {
        ...
        if (status == 0) {
            ...
            if (path != NULL) {
                ...
                if (status != INSTALL_SUCCESS) {
                    ui->DialogShowErrorLog("很抱歉，安装系统更新包失败 ...");
                }
            }
            else {
                 ui->Print("\n-- 没有选择系统更新包 ...\n", path);
    ...
    if (status == INSTALL_SUCCESS && wipe_cache) {
        ui->Print("\n-- 擦除缓存 (at package request)...\n");
        ui->DialogShowInfo("擦除缓存中 ...");
    ...
    if (status >= 0 && status != INSTALL_NONE) {
        if (status != INSTALL_SUCCESS) {
            ui->SetBackground(RecoveryUI::ERROR);
            ui->Print("很抱歉，系统更新包安装失败 ...\n");
            ui->DialogShowErrorLog("很抱歉，系统更新包安装失败 ...");
        } else if (ui->IsTextVisible()) {
            ui->Print("\n安装系统更新包成功 ...\n");
        }
    }
...
main(int argc, char **argv) {
    ...
    if (oem_lock == OEM_LOCK_UNLOCK) {
        ...
        if (status != INSTALL_SUCCESS) ui->Print("擦除数据失败.\n");
    ...
    if (update_package != NULL) {
        ...
        if (status == INSTALL_SUCCESS && wipe_cache) {
            if (erase_volume("/cache")) {
                LOGE("擦除缓存 (requested by package) 失败.");
            }
        }
        if (status != INSTALL_SUCCESS) {
            ui->Print("很抱歉,系统更新包安装失败 ...\n");
        ...
        if (wipe_cache && erase_volume("/cache")) status = INSTALL_ERROR;
        if (status != INSTALL_SUCCESS) ui->Print("擦除缓存失败.\n");
    } else if (wipe_media) {
        if (erase_volume("media")) status = INSTALL_ERROR;
        if (status != INSTALL_SUCCESS) ui->Print("擦除内置存储失败.\n");
    } else if (sideload) {
        status = enter_sideload_mode(&wipe_cache, device);
    } else if (!just_exit) {
